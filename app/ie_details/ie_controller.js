app.controller('IEServiceCtrl', ['$scope','CurrentServices',function($scope, CurrentServices) {
    ...
    */
    $scope.getTrafficData = function(startTime, endTime, size) {
      ...
      CurrentServices.getTrafficData($scope.uaaToken, startTime, endTime, size).then(function(data){
            ...
            if(event && event.measures) {
              for(var idx = 0; idx < event.measures.length; idx++) {
                ...
                if(measure.tag === 'direction') {
                  trafficData.direction = $scope._getDirectionByDegree(measure.value);
                } else if (measure.tag === 'speed') {
                  trafficData.speed = measure.value + ' MPH';
                } else if (measure.tag === 'vehicleCount') {
                  trafficData.count = measure.value;
                }
              }
            }
        }
