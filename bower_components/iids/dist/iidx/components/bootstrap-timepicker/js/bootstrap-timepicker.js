	...
	Timepicker.prototype = {
		...
		setTime: function (time, ignoreWidget) {
			...
			if (typeof time === 'object' && time.getMonth) {
				...
				// Special invalidation rules:
				if(this.explicitMode) {

					/* Allowed patterns:
					1, 12, 1234, 123456, 1:2:3, 12:34:56, 12:3:45
					a, p, am, pm (case insensitive)
					*/
					var acceptablePattern = /^\s*((\d\d){1,3}|(\d?\d((\s*[:]\s*)\d?\d){0,2}))(\s*[ap]m?)?\s*$/i;
					if(!acceptablePattern.test(time)) {
						this.invalidate();
						return;
					}
				...
				if(timeArray.length<2)
				{
					...
					if(hour.length>6)
					{
						...
						second = hour.substring(4);
						minute = hour.substring(2,4);
					...
					} else
					{
						...
						if(hour.length>4)
						{
							second = hour.slice(-2); // Takes the last two digits off the end and makes them seconds.
							hour = hour.slice(0, -2); // Takes the all the digits except the last two.
						}
				...
				if(timeArray.length<3)
				{
					if(minute.length>4)
					{
						...
						if(minute.length>2)
						{
							second = minute.slice(-2); // Takes the last two digits off the end and makes them seconds.
							minute = minute.slice(0, -2); // Takes the all the digits except the last two.
						}
					}
				}
